...
// shouldBeInHook mirrors eslint-plugin-jest's shouldBeInHook.
// NOTE: ExpressionStatement only recurses one level into its expression. A
// parenthesized call like `(setup())` is a ParenthesizedExpression and falls
// through to the default arm (not flagged), matching upstream.
func shouldBeInHook(node *ast.Node, ctx rule.RuleContext, allowedFunctionCalls []string) bool {
	...
	case ast.KindVariableStatement:
		...
		// const is always allowed; let/var with a non-null/undefined initializer are not.
...
// describeCallbackBody returns the block body of a describe(...) callback when
// `statement` is an ExpressionStatement calling a describe-like Jest function.
func describeCallbackBody(statement *ast.Node, ctx rule.RuleContext) *ast.Node {
	...
	if !utils.IsTypeOfJestFnCall(expr, ctx, utils.JestFnTypeDescribe) {
		return nil
	}
	args := expr.AsCallExpression().Arguments
...
// checkBlockBody reports statements that should be in a hook, then recursively
// walks describe callback bodies in source order so diagnostics match upstream
// location ordering (Program + nested describe, not CallExpression-after-Program).
func checkBlockBody(ctx rule.RuleContext, body []*ast.Node, allowedFunctionCalls []string) {
	for _, statement := range body {
		...
		if block := describeCallbackBody(statement, ctx); block != nil && block.AsBlock().Statements != nil {
			checkBlockBody(ctx, block.AsBlock().Statements.Nodes, allowedFunctionCalls)
		}
	}
}
